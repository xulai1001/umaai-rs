...
class TrainersLegendTranslator(Translator):
    ...
    def _ensure_translate_mapping(self):
        if self.translate_mapping:
            return
        target_file = f"text_data_dict.json"
        self.translate_mapping = get_json_from_github_file(
    ...
    def translate_chara_card(self, chara_card:CharacterCard) -> CharacterCard:
        if self._has_translate("4", chara_card.id):
            chara_card.name = self._translate(chara_card.name, "4", chara_card.id)
            chara_card.is_translated = True
        else: 
            char_id = str(int(chara_card.id) // 100)
            if self._has_translate("6", char_id): 
                parts = chara_card.name.split("]")
                chara_card.name = f"{parts[0]}]{self._translate(parts[1], "6", char_id)}"
                chara_card.is_translated = True
        return chara_card
    ...
    def translate_support_card(self, support_card:SupportCard) -> SupportCard:
        ...
        if not support_card.is_translated:
            nick_name, real_name = self._extract_name(support_card.name)
            real_name = self._translate(real_name, "6", str(support_card.chara_id))
            # print(">>", nick_name, real_name, support_card.chara_id)
            if nick_name:
                support_card.name = f"[{nick_name}]{real_name}"
            else:
                support_card.name = real_name
            support_card.is_translated = self._has_translate("6", str(support_card.chara_id))
        return support_card
